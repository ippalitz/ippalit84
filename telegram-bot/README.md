...
### Согласие клиента и конфиденциальность

До нажатия кнопки `Согласен, включить ИИ` сообщения, VIN и фотографии клиента в OdiRouter не отправляются. При отказе бот продолжает работать через ADEO и живого менеджера без внешнего ИИ. Согласие хранится только в памяти процесса и после перезапуска Render запрашивается снова.
